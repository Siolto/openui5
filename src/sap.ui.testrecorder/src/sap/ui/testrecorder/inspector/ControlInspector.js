...
sap.ui.define(
	...
	function (
		...
		_ControlFinder,
		ControlAPI,
		ControlInspectorRepo,
		constants,
		DialectRegistry,
		Dialects,
		ControlSelectorGenerator,
		POMethodUtil,
		RawSnippetUtil,
		CodeSnippetProvider,
		SharedModel
		...
		var mSelectorSettings = Object.assign(
			{},
			SharedModel.getData().settings
		);
		 ...
		 */
		var ControlInspector = BaseObject.extend(
			"sap.ui.testrecorder.inspector.ControlInspector",
			{
				constructor: function () {
					...
					if (!oControlInspector) {
						...
						this._appObserver = new AppMutationObserver(
							this.getAllControlData.bind(this)
						);
						this._selectedElementObserver =
							new ElementMutationObserver(
								this.getControlData.bind(this)
							);
					} else {
		...
		ControlInspector.prototype.init = function () {
			...
			CommunicationBus.subscribe(
				CommunicationChannels.REQUEST_ALL_CONTROLS_DATA,
				this.getAllControlData.bind(this)
			);
			CommunicationBus.subscribe(
				CommunicationChannels.REQUEST_CONTROL_DATA,
				this.getControlData.bind(this)
			);
			CommunicationBus.subscribe(
				CommunicationChannels.REQUEST_CODE_SNIPPET,
				this.getCodeSnippet.bind(this)
			);
			CommunicationBus.subscribe(
				CommunicationChannels.HIGHLIGHT_CONTROL,
				this.highlightControl.bind(this)
			);
			CommunicationBus.subscribe(
				CommunicationChannels.SET_DIALECT,
				this.setDialect.bind(this)
			);
			CommunicationBus.subscribe(
				CommunicationChannels.UPDATE_SETTINGS,
				this.updateSettings.bind(this)
			);
			CommunicationBus.subscribe(
				CommunicationChannels.CLEAR_SNIPPETS,
				this.clearSnippets.bind(this)
			);
		};
		...
		ControlInspector.prototype.getAllControlData = function () {
			CommunicationBus.publish(
				CommunicationChannels.RECEIVE_ALL_CONTROLS_DATA,
				{
					renderedControls:
						ControlAPI.getAllControlData().renderedControls,
		...
		ControlInspector.prototype.getControlData = function (mData) {
			var oDomElement = mData.domElementId
				? document.getElementById(mData.domElementId)
				: sap.ui.getCore().byId(mData.controlId).getDomRef();
			...
			CommunicationBus.publish(
				CommunicationChannels.RECEIVE_CONTROL_DATA,
				mControlData
			);
		};
		...
		ControlInspector.prototype.getCodeSnippet = function (mData) {
			...
			var mControlSelector = ControlInspectorRepo.findSelector(
				mData.domElementId
			);
			var oSelectorPromise = mControlSelector
				? Promise.resolve(mControlSelector)
				: ControlSelectorGenerator.getSelector(mDataForGenerator);

			return oSelectorPromise
				...
				.then(function (sSnippet) {
					...
					var aSnippets = mSelectorSettings.multipleSnippets
						? ControlInspectorRepo.getSnippets()
						: [sSnippet];
					...
					if (DialectRegistry.getActiveDialect() === Dialects.RAW) {
						return RawSnippetUtil.getJSON(
							aSnippets,
							mSelectorSettings
						);
					} else {
						return POMethodUtil.getPOMethod(
							aSnippets,
							$.extend(
								{
									action: mData.action,
							...
							DialectRegistry.getActiveDialect() === Dialects.WDI5
						);
					}
				})
				.then(function (sSnippet) {
					...
					CommunicationBus.publish(
						CommunicationChannels.RECEIVE_CODE_SNIPPET,
						{
				...
				.catch(function (oError) {
					CommunicationBus.publish(
						...
						{
							error:
								"Could not generate code snippet for " +
		...
		ControlInspector.prototype.highlightControl = function (mData) {
			if (mData.domElementId) {
				...
				var domElement = _ControlFinder._findElements({
					id: mData.controlId
				})[0];
				if (domElement) {
		...
		ControlInspector.prototype.setDialect = function (sDialect) {
			if (DialectRegistry.getActiveDialect() !== sDialect) {
				...
				CommunicationBus.publish(
					CommunicationChannels.DIALECT_CHANGED,
					{
						dialect: sDialect
					}
				);
				ControlInspectorRepo.getRequests().forEach(
					this.getCodeSnippet.bind(this)
				);
			}
		};
